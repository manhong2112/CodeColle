...
MemberInfo = namedtuple("MemberInfo", ["mid", "name", "pageNum", "illustNum", "illustList"])
...
def parse_member_id(mid, cookie):
    ...
    for i in range(1, info.pageNum + 1):
        ...
        info.illustList.extend(get_illust_id(GET(url, headers={"Cookie": cookie}).read().decode("utf-8")))
    return info
