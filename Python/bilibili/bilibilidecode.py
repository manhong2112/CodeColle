...
def get_info(aid):
    video_info = [get_json("{0}/view?type=json&appkey={1}&id={2}".format(api_1, api_key, aid))]
    pages_num = video_info[0]['pages']
    if pages_num > 1:
        video_info = []
        for i in range(1, pages_num + 1):
            time.sleep(0.2)
            video_info.append(get_json("{0}/view?type=json&appkey={1}&id={2}&page={3}".format(api_1, api_key, aid, i)))
    return video_info


def print_info(v_info):
    cid = v_info['cid']
    link_info = get_json('{0}/playurl?type=mp4&otype=json&quality=4&appkey={1}&cid={2}'.format(api_2, api_key, cid))
    try:
        printf(">> cid: {0}", cid)
        printf(">> Title: {1} Author: {0[author]} Upload Time: {0[created_at]}", v_info, html.unescape(v_info['title']))
        printf(">> Play: {0[play]} Coins: {0[coins]} Fav: {0[favorites]} Pages: {0[pages]}", v_info)
        printf(">> Type: {0[typename]} Tag: {1}", v_info, v_info['tag'].split(','))
        printf(">> Description: {0}", html.unescape(v_info['description']))
        printf(">> Danmu file link at: {0}/{1}.xml", api_3, cid)
        printf(">> Video link at bilibilijj: {0}/freedown/spare/{1}.mp4", bilibilijj, cid)
        try:
